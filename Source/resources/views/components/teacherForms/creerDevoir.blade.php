<div id="CreateAssignementModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
  <div class="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-md relative">
      <!-- Close Button Improvement -->
      <button id="closeModalTrigger" data-modal-id="CreateAssignementModal" class="absolute top-4 right-4 text-gray-500 hover:text-blue-600 transition-colors">
          <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
          </svg>
      </button>
      <h2 class="text-2xl font-bold text-slate-800 mb-6">Créer un nouveau cours</h2>
      <form action="/create/course" method="post" class="space-y-6">
          @csrf
          <!-- Input Field Styling -->
          <div>
              <label class="block text-sm font-medium text-slate-700 mb-2">Nom de la classe</label>
          ...
          <div>
              ...
              <select id="professoremail" name="professoremail" 
